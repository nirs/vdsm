...
# Collect tests logs.
# NOTE: should be called only in teardown context. Always succeeds, even if
# collectring logs failed.
collect_logs()
{
    # NOTE: Tar fails randomly when some log file is modified while tar is reading
    # it, and there is no way to detet and filter this failure.  We also do not
    # want to fail the build if log collections failed.
